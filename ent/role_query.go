...
func (rq *RoleQuery) sqlAll(ctx context.Context) ([]*Role, error) {
	...
	if query := rq.withUsers; query != nil {
		...
		for _, n := range neighbors {
			fk := n.role_users
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "role_users" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "role_users" returned %v for node %v`, *fk, n.ID)
			}
