...
func (eq *EquipmentQuery) QueryKind() *KindQuery {
	query := &KindQuery{config: eq.config}
...
func (eq *EquipmentQuery) QueryStatus() *StatusesQuery {
...
func (eq *EquipmentQuery) Clone() *EquipmentQuery {
	...
	return &EquipmentQuery{
		...
		withKind:   eq.withKind.Clone(),
		withStatus: eq.withStatus.Clone(),
...
func (eq *EquipmentQuery) WithKind(opts ...func(*KindQuery)) *EquipmentQuery {
	query := &KindQuery{config: eq.config}
...
func (eq *EquipmentQuery) WithStatus(opts ...func(*StatusesQuery)) *EquipmentQuery {
	query := &StatusesQuery{config: eq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EquipmentQuery) sqlAll(ctx context.Context) ([]*Equipment, error) {
	...
	if eq.withKind != nil || eq.withStatus != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := eq.withKind; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Equipment)
		for i := range nodes {
			if nodes[i].kind_equipments == nil {
				continue
			}
			fk := *nodes[i].kind_equipments
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "kind_equipments" returned %v`, n.ID)
			}
			for i := range nodes {
	...
	if query := eq.withStatus; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Equipment)
		for i := range nodes {
			if nodes[i].statuses_equipments == nil {
				continue
			}
			fk := *nodes[i].statuses_equipments
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(statuses.IDIn(ids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "statuses_equipments" returned %v`, n.ID)
			}
			for i := range nodes {
